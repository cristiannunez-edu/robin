...
if ("undefined" != typeof responsiveVoice) console.log("ResponsiveVoice already loaded"), console.log(responsiveVoice);
            ...
            if (a.rvsMapped) {
                ...
                if (a.iOS9 && !a.iOS9_initialized) a.log("Initializing ios9"), setTimeout(function() {
                    ...
                    for (g = 0; g < k.length; g++)
                        if (!a.fallbackMode && a.getServiceEnabled(a.services.NATIVE_TTS)) a.log("Using SpeechSynthesis"), h = a.services.NATIVE_TTS, e = new SpeechSynthesisUtterance, e.voiceURI = f.systemvoice.voiceURI, e.volume = a.selectBest([f.collectionvoice.volume,
        ...
        a.getWords = function(b) {
            for (var c = b.split(/(\s*[\s,]\s*|\?|;|:|\.|\(|\)|!)/), c = c.filter(function(a) {
                    return /[^\s]/.test(a)
                }), d = 0; d < c.length; d++) null != (b = c[d].toString().match(/\d+/)) && (c.splice(d, 1), a.numToWords(+b[0]).split(/\s+/).map(function(a) {
                c.push(a)
            }));
            return c
        };
